...
*   **Fricción de UX y Delay Artificial de Carga:** Para maximizar la visibilidad y el CTR (Click-Through Rate) de los anuncios en pantalla, se ha implementado un retraso artificial global de **2.5 segundos** en el procesamiento local de archivos. Este retardo se inyecta en `theme.js` interceptando los métodos nativos `HTMLCanvasElement.prototype.toBlob` y `jsPDF.prototype.save`. Esto mantiene la barra de carga visible durante unos segundos mientras la atención del usuario está capturada y el anuncio se renderiza por completo.

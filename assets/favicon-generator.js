...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function addFileToList(file) {
        ...
        convertBtn.addEventListener('click', () => {
            generateFaviconICO(fileWrapper);
        });
    ...
    async function generateFaviconICO(fileWrapper) {
        ...
        img.onload = async () => {
            try {
                ...
                // Define standard resolutions for favicon.ico
                ...
                // Pack PNG binaries into a single ICO file
                ...
                fileWrapper.convertedBlobUrl = icoUrl;
                fileWrapper.convertedBlobName = icoName;
                ...
                setTimeout(() => {
                    ...
                    downloadBtn.href = icoUrl;
                    downloadBtn.download = icoName;
    ...
    async function packIco(pngBlobs, resolutions) {
        ...
        view.setUint16(2, 1, true);      // Resource Type (1 for ICO)
        view.setUint16(4, count, true);  // Number of images

        let currentOffset = headerSize;

        // ICONDIRENTRY and Raw PNG Data Copy
        for (let i = 0; i < count; i++) {
            const img = images[i];
            ...
            icoBytes[entryOffset + 2] = 0; // Number of colors
            icoBytes[entryOffset + 3] = 0; // Reserved
            view.setUint16(entryOffset + 4, 1, true); // Color Planes
            view.setUint16(entryOffset + 6, 32, true); // Bits per pixel
            view.setUint32(entryOffset + 8, img.size, true); // Size of image data
            view.setUint32(entryOffset + 12, currentOffset, true); // Offset to image data

            // Copy binary PNG data
    ...
    if (processBtn) {
        processBtn.addEventListener('click', () => {
            ...
            if (pending.length > 0) {
                playPopSound();
                pending.forEach(f => generateFaviconICO(f));
            }
        });
    }

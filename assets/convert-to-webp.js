/**
 * My Local Picture - Client-side Convert to WebP
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleFiles(files) {
        ...
        Array.from(files).forEach(file => {
            ...
            const supported = ['jpg', 'jpeg', 'png', 'webp', 'bmp'].includes(ext) || file.type.startsWith('image/');
        ...
        if (rejectedCount > 0) {
            const warningMsg = isEnglish 
                ? `Skipped ${rejectedCount} file(s) because they were not supported image formats.`
    ...
    function convertToWebp(fileWrapper) {
        ...
        img.onload = () => {
            ...
            // Draw image on canvas (WebP supports transparency, so no solid background required)
            ...
            setTimeout(() => {
                canvas.toBlob((blob) => {
                    if (blob) {
                        ...
                        const webpName = fileWrapper.originalName.replace(/\.[^/.]+$/, "") + '.webp';

                        fileWrapper.status = 'done';
                        fileWrapper.convertedBlobUrl = webpUrl;
                        fileWrapper.convertedBlobName = webpName;
    ...
    if (clearAllBtn) {
        ...
        convertAllBtn.addEventListener('click', () => {
            ...
            if (pending.length > 0) {
                playPopSound();
                pending.forEach(f => convertToWebp(f));
            }
        });
    }
